...
IF ERRORLEVEL 5 set option5=%option5%%mark_on% && goto 5
IF ERRORLEVEL 4 set option4=%option4%%mark_on% && goto 4
IF ERRORLEVEL 3 set option3=%option3%%mark_on% && goto 3
IF ERRORLEVEL 2 set option2=%option2%%mark_on% && goto 2
IF ERRORLEVEL 1 set option1=%option1%%mark_on% && goto 1

REM	fals eine Abfrage nicht abgefangen wurde, laeuft die Schleife weiter
echo Auswahl nicht erkannt!
goto anfang
...
IF  %option1%==%mark_off%%mark_on% (
	set option1=%mark_on%
	echo Schleife "IF" Status Option1: %option1%
) ELSE (
	set option1=%mark_off%
	echo Schleife "ELSE" Status Option1: %option1%
)
echo Optionen Inhalt: %option1%
pause
goto anfang
